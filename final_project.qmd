...
# one_word_vehicle_model = c("ILX", "TLX", "")

two_word_vehicle_model = c("RDX AWD", "MDX SH-AWD", "TLX FWD", "TLX SH-AWD", "124 Spider", "1500 4WD", "1500 4x4", "")
...
# 
# temp = unique(all_data_15_24)
# View(temp) all_data_15_24


# Perform approximate string matching
mapped_data <- fuzzyjoin::stringdist_join(
  final_vehicle_prices, all_data_15_24,
  by = "model",
  method = "jw",
  max_dist = 0.27,
  mode = "left",
...
final_vehicle_prices = final_vehicle_prices %>% 
  ...
  mutate(merged_used_new_price = if_else(!is.na(avg_used_price), avg_used_price, avg_new_price))


# View the result
View(final_vehicle_prices)

vehicle_used_prices = final_vehicle_prices %>% filter(!is.na(avg_used_price))
vehicle_new_prices = final_vehicle_prices %>% filter(!is.na(avg_new_price))

vehicle_used_prices = vehicle_used_prices %>% 
  rename(
   model_year_old = model_year
  )

vehicle_new_prices = vehicle_new_prices %>% 
  rename(
   model_year_new = model_year
  )

View(vehicle_new_prices)

final_vehicle_prices = vehicle_new_prices %>% 
  filter(model_year_new >= 2016)|> 
  select(model_year_new, make, model, avg_new_price) %>% 
  full_join(vehicle_used_prices %>% select(model_year_old, make, model, avg_used_price, avg_mileage),
            by = c("make", "model")) %>% 
  arrange(make, model) %>% 
  select(model_year_new, model_year_old, make, model, avg_new_price, avg_used_price, avg_mileage)
...
# final_vehicle_prices$model_year = as.integer(final_vehicle_prices$model_year)
...
complete_dataset = complete_dataset %>% 
  select(model_year_new, model_year_old, make, model, vehicle_class, avg_used_price, avg_new_price, avg_mileage, motor_kw, engine_size_l,
         cylinders, transmission, fuel_type, year, month, average_fuel_price,
         avg_fuel_price_yr, avg_fuel_type_price, recharge_time_h, city_l_100_km, 
         highway_l_100_km, combined_l_100_km, city_kwh_100_km, highway_kwh_100_km, 
         combined_kwh_100_km, range_km, combined_mpg, co2_emissions_g_km, 
